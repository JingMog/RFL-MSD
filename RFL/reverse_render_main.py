...
def do_single_task(params, args, records_queue, records_queue_lock, shared_params={}):
    ...
    if args.input_type == "text":
        ...
        out_text = reverse_render.reverse_organic_trans(in_text, debug=False)
        out_text = " ".join(out_text)
        out_text = utils.process_trans_for_texlive(out_text)
        with out_lines_lock:
        ...
        while top_id < len(areas_arr):
            ...
            if "text" in cur_area:
                ...
                cur_area["text"] = text_render.text_render(in_text, debug=True)
            ...
            if args.input_type == "lrc":
                try:
                    with shared_params["out_records_lock"]:
                        cur_record = shared_params["out_records"].get()
