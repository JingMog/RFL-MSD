...
def process_cond_render(text, preprocess=True):
    chemfig_text, rep_dict, rep_text = utils.replace_chemfig(text)
    rep_text_units = parse_transcription(rep_text, simple_trans=True) # 汉字->UC
    new_rep_dict = {}
    for key, value in rep_dict.items():
        # chemfig_str = "".join(list(filter(lambda x:x!=" ", value)))
        chemfig_str = value.strip()[8:].strip()[1:-1]
        try:
            rep_atom = chemfig_cond_render.chemfig_random_cond_parse(chemfig_str, preprocess=preprocess, bond_dict=[])
...
def main(args):
    ...
    if args.input_type == "text":
        ...
        while not shared_params["out_records"].empty():
            record = shared_params["out_records"].get()
            lrc_writer.add_record(record)
        #pdb.set_trace()
        lrc_writer.close()
